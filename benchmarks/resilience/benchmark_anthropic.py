...
def configure_resilience_suite() -> None:
    """Point the shared Anthropic runner at the independently verified suite."""
    benchmark.BENCHMARK_VARIANT = "resilience_v1"
    benchmark.RESULTS_BASENAME = "resilience_bedrock_reasoning_benchmark_anthropic"
    benchmark.REPAIR_RESULTS_BASENAME = "resilience_bedrock_reasoning_repair_anthropic"
    ...
    benchmark.EXPECTED_ANSWERS = load_expected_answers(
        (scenario_id for scenario_id, _ in benchmark.PROMPTS), ANSWER_KEY_PATH
    )
    benchmark.verify_answer_key = verify_answer_key


if __name__ == "__main__":
    configure_resilience_suite()
    benchmark.main()
